...
def gather_feats(files, output_path):
    ...
    #debugging
    lmdb_feats = lmdb.open(str(output_path), 2*1024**3, writemap=True)#lmdb_feats = lmdb.open(str(output_path), 700*1024**3, writemap=True)
    #end
...
def gather_masks(files, output_path):
    ...
    if output_path.is_dir():
        shutil.rmtree(output_path)
    # debugging
    lmdb_masks = lmdb.open(str(output_path), 1*1024**3, writemap=True)# lmdb_masks = lmdb.open(str(output_path), 50*1024**3, writemap=True)
    # end
