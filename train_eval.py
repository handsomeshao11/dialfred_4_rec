...
def evalModel(args):
    ...
    # check_list=[model_args, lang, dataset, encoder, decoder, critic, performer, extractor, all_ans]
    # # print(f"""Memory used by \n
    # #       model_args: {asizeof.asizeof(model_args)*(10**-9)} GBs\n
    # #       lang: {asizeof.asizeof(lang)*(10**-9)} GBs\n
    # #       dataset: {asizeof.asizeof(dataset)*(10**-9)} GBs\n
    # #       encoder: {asizeof.asizeof(encoder)*(10**-9)} GBs\n
    # #       decoder: {asizeof.asizeof(decoder)*(10**-9)} GBs\n
    # #       critic: {asizeof.asizeof(critic)*(10**-9)} GBs\n
    # #       performer: {asizeof.asizeof(performer)*(10**-9)} GBs\n
    # #       extractor: {asizeof.asizeof(extractor)*(10**-9)} GBs\n
    # #       all_ans: {asizeof.asizeof(all_ans)*(10**-9)} GBs\n
    # #       """)
    
    # with open("evaliters_args.pickle","wb") as f:
